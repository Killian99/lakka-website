<h2>On which hardware do you want to install Lakka?</h2>

<p>Currently, only the Raspberry Pi is supported.</p>

<ul class="platforms clearer">
	<li class="rpi"><a href="/getlakka?step=3&amp;os=<?php echo $_GET['os']; ?>&amp;platform=rpi">
		<img src="images/rpi.jpg" />
		<h3>Raspberry Pi</h3>
		<p>Stable</p>
	</a></li>
	<li class="x86_64 disabled"><a href="">
		<img src="images/nuc.jpg" />
		<h3>PC</h3>
		<p>Coming soon</p>
	</a></li>
	<li class="atv disabled"><a href="">
		<img src="images/atv.jpg" />
		<h3>Apple TV</h3>
		<p>Coming soon</p>
	</a></li>
	<li class="cuboxi disabled"><a href="">
		<img src="images/cuboxi.png" />
		<h3>Cubox-i</h3>
		<p>Coming soon</p>
	</a></li>
	<li class="cubie disabled"><a href="">
		<img src="images/cubie1.jpg" />
		<h3>Cubieboard</h3>
		<p>Coming soon</p>
	</a></li>
	<li class="cubie disabled"><a href="">
		<img src="images/cubie2.jpg" />
		<h3>Cubieboard 2</h3>
		<p>Coming soon</p>
	</a></li>
	<li class="cubie disabled"><a href="">
		<img src="images/cubie3.png" />
		<h3>Cubietruck</h3>
		<p>Coming soon</p>
	</a></li>
	<li class="radxa disabled"><a href="">
		<img src="images/radxa_rock.png" />
		<h3>Radxa</h3>
		<p>Coming soon</p>
	</a></li>
</ul>
